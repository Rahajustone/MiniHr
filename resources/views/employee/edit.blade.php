    ...
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <form class="forms-sample" action="{{ route('employee.update', [ 'id' => $employee->id ])}}" method="POST">
                    @method('put')
                    @csrf
                    <div class="form-group">
                        <label for="exampleInputName1">First Name</label>
                        <input type="text" class="form-control" id="exampleInputName1" placeholder="First Name" name="name" required value="{{ $employee->name }}">
                    </div>
                    <div class="form-group">
                        <label for="exampleInputLastName1">Last Name</label>
                        <input type="text" class="form-control" id="exampleInputLastName1" placeholder="Last Name" name="surname" required value="{{ $employee->surname }}">
                    </div>
                    <div class="form-group">
                        <label for="exampleInputName1">Birthday</label>
                        <input type="date" class="form-control" id="exampleInputName1"  name="birthday" required value="{{ date('m/d/Y', strtotime($employee->birthday)) }}">
                    </div>
                    <div class="form-group">
                        <label for="exampleInputName1">Salary</label>
                        <input type="number" step="any" class="form-control" id="exampleInputName1" placeholder="Salary" name="salary" required value="{{ $employee->salary }}">
                    </div>
                    <div class="form-group">
                        ...
                        <div class="">
                            <select class="form-control" name="department_id">
                                ...
                                @if(count($departments)>0)
                                    ...
                                    @endforeach
                                @else
                                <option disabled>Please Add a Department</option>
                                @endif
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="Experience" rows="4" name="experience" required>{{ $employee->experience }}</textarea>
                    </div>
